...
def build_role_prompt(role_config, **kwargs):
    base_template = (
    ...
    # 动态添加分析阶段内容
    if "prev_analysis" in kwargs:
        base_template += "前期分析：{prev_analysis}\n"
    if "analyses" in kwargs:
        base_template += "分析汇总：\n{analyses}\n"
    
    base_template += "<|eot_id|><|start_header_id|>assistant<|end_header_id|>\n\n"
...
def format_prompt(
    ...
    docs_text = "\n".join(
        [f"Doc {idx+1} (Title: {ctx['title']}): {ctx['text']}" 
    ...
    # Incorporate multi-role analysis if exists
    if 'domain_analysis' in example:
        query_prompt += f"\n\nDomain Analysis: {example['domain_analysis']}"
        
        # 如果存在辩论摘要，添加进去
        if 'debate_summary' in example:
            query_prompt += f"\n\nExperts Debate: {example['debate_summary']}"
            if 'final_answer' in example:
                query_prompt += f"\n\nExperts' Final Answer: {example['final_answer']}"
        # 兼容旧版分析结果
        elif 'reasoning_steps' in example:
            query_prompt += f"\nReasoning Steps: {example['reasoning_steps']}"
            if 'final_synthesis' in example:
                query_prompt += f"\nSynthesized Analysis: {example['final_synthesis']}"

    if do_rationale_generation:
        query_prompt = ''
        prefix += prompt_dict['demo_prefix'].format_map(example)
        target_prefix += (
            prompt_dict['rationale_generation_instruction'].format_map(example) + 
            prompt_dict[f'rationale_generation_postfix_{dataset_name}']
        )
    elif len(demos) > 0:
    ...
    content = f"{docs_text}\n\n{query_prompt}{target_prefix}"
    content_ids = tokenizer(content, return_tensors="pt", add_special_tokens=False).input_ids

    if content_ids.shape[-1] > max_length - prefix_len:
        content_ids = content_ids[..., -(max_length - prefix_len):]

    input_ids = torch.cat([prefix_ids, content_ids], axis=-1)
